...
function QuestPageInner() {
  ...
  useEffect(() => {
    ...
    if (nextSet.length !== quizSize && poolCandidates.length > 0) {
      const emergencyBase = shuffle(poolCandidates).slice(0, Math.min(poolCandidates.length, quizSize));
      if (emergencyBase.length > 0) {
        const emergencySet: QuestEntry[] = [...emergencyBase];
        while (emergencySet.length < quizSize) {
          emergencySet.push(emergencyBase[emergencySet.length % emergencyBase.length]);
        }
        nextSet = shuffle(emergencySet).slice(0, quizSize);
      }
    }
