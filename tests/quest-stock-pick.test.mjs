...
test("pick quiz from stock uses fisher-yates shuffle + slice", () => {
  ...
  assert.equal(source.includes("let picked: QuestEntry[];"), true);
  assert.equal(source.includes("picked = shuffle(deduped).slice(0, Math.min(requested, availableAfterDedupe));"), true);
  assert.equal(source.includes("if (targetDifficulty == null)"), true);
  assert.equal(source.includes("pickQuizByDifficulty("), true);
...
test("quest page renders shortage list when stock count is low", () => {
  assert.equal(questSource.includes("const [stockShortages, setStockShortages] = useState<any[]>([]);"), true);
