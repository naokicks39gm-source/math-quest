...
test("quest page exposes fraction TeX helpers for integer and exponent fractions", () => {
  ...
  assert.match(source, /const INTEGER_FRACTION_PATTERN = \/\(\[\+\-\]\?\\d\+\)\\\/\(\[\+\-\]\?\\d\+\)\/g;/);
  assert.match(source, /const EXPONENT_FRACTION_PATTERN = \/\(\[/);
  assert.equal(source.includes('if (!raw.includes("^")) continue;'), true);
...
test("quest card uses responsive layout to avoid answer overflow", () => {
  ...
  assert.match(source, /useSingleLineQa[\s\S]*\? "relative z-10 w-full flex flex-wrap items-start justify-start gap-5  sm:gap-3"/);
  assert.match(source, /useSingleLineQa[\s\S]*: "relative z-10 w-full flex flex-col justify-start gap-1 pt-1 sm:gap-2"/);
...
test("fraction renderer keeps num/den shape", () => {
  const source = read("src/app/quest/utils/renderFraction.tsx");
  assert.match(source, /renderFractionEditorValue = \(editor: FractionEditorState \| null \| undefined\)/);
  assert.match(source, /renderPart\(editor\.num, editor\.part === "num"\)/);
  assert.match(source, /renderPart\(editor\.den, editor\.part === "den"\)/);
});
