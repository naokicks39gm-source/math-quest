...
const loadProblemEngineModules = async () => {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "difficulty-range-"));
...
test("all generated problems keep difficulty in the 1-5 range", async () => {
  ...
  for (const gradeType of gradeTypes) {
    ...
    for (const pattern of patterns) {
      ...
      for (const problem of generated) {
        const difficulty = problem.meta?.difficulty;
        assert.equal(typeof difficulty, "number", `${pattern.key} must set difficulty`);
        assert.equal(difficulty >= 1 && difficulty <= 5, true, `${pattern.key} difficulty out of range: ${difficulty}`);
      }
    }
  }
});
