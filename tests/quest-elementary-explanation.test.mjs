...
test("elementary explanation resolver has abacus/column_story/simple branches", () => {
  ...
  assert.match(libSource, /groupSize\?: number/);
  assert.match(libSource, /groupedTotal\?: number/);
  ...
  assert.match(libSource, /leadText\?: string/);
  assert.match(libSource, /tableRows\?: Array<\{ expr: string; goro: string \}>/);
  assert.match(libSource, /const normalizeAnswerText =/);
  assert.match(libSource, /const E2_DAN_PATTERN_RE = \/\^MUL_1D_1D_DAN_\(\[1-9\]\)\$\//);
  assert.match(libSource, /const E2_DAN_GORO: Record<number, string\[]> =/);
  assert.match(libSource, /const isE2Mul99Type =/);
  assert.match(libSource, /const resolveDanFromPrompt =/);
  assert.match(libSource, /const parseDanGoroLine =/);
  assert.match(libSource, /const buildE2DanGoroAid =/);
  assert.match(libSource, /title: `\$\{dan\}のだん ごろあわせ`/);
  assert.match(libSource, /leadText/);
  assert.match(libSource, /tableRows/);
  assert.match(libSource, /if \(gradeId === "E2" && dan !== null && isE2Mul99Type\(typeId, patternId\)\)/);
  assert.match(libSource, /return buildE2DanGoroAid\(dan, prompt\);/);
  ...
  assert.match(libSource, /if \(patternId === "DIV_EQUAL_SHARE_BASIC"\)/);
  assert.match(libSource, /buildEqualShareAbacusAid/);
  assert.match(libSource, /title: "おはじきでわける"/);
  assert.match(libSource, /steps: \[\]/);
  assert.match(libSource, /leadText: `\$\{total\}こ\$\{people\}人にわける`/);
  assert.match(libSource, /groupSize: people/);
  assert.match(libSource, /groupedTotal: total/);
  assert.match(libSource, /return buildEqualShareAbacusAid\(Math\.abs\(parsed\.left\), Math\.abs\(parsed\.right\)\);/);
  ...
  assert.match(libSource, /focusPlace: "tens"/);
  assert.match(libSource, /focusPlace: "hundreds"/);
...
test("elementary panel has next button and visual rendering", () => {
  ...
  assert.match(panelSource, /aid\.leadText && \(/);
  assert.match(panelSource, /aid\.tableRows && aid\.tableRows\.length > 0/);
  assert.match(panelSource, /<table/);
  assert.match(panelSource, /しき/);
  assert.match(panelSource, /ごろあわせ/);
  assert.match(panelSource, /aid\.visual\?\.mode === "abacus"/);
  assert.match(panelSource, /aid\.visual\.groupSize && aid\.visual\.groupedTotal/);
  assert.match(panelSource, /makeGroups\(aid\.visual\.groupedTotal, aid\.visual\.groupSize\)/);
  assert.match(panelSource, /rounded-full border border-emerald-300/);
  assert.match(panelSource, /aid\.visual\.groupSize\}こずつ まるで かこむ/);
  assert.match(panelSource, /aid\.visual\.operator === "÷"/);
  assert.match(panelSource, /"わける"/);
  assert.match(panelSource, /"1にんぶん"/);
  ...
  assert.match(panelSource, /const isColumnExplanation = aid\.visual\?\.mode === "column" \|\| aid\.visual\?\.mode === "column_story";/);
  assert.match(panelSource, /aid\.steps\.length > 0 && !isColumnExplanation/);
  ...
  assert.match(panelSource, /inline-flex items-center/);
  assert.match(panelSource, /min-w-\[0\.68ch\] mx-\[0\.10ch\]/);
  ...
  assert.match(panelSource, /aid\.visual\.operator === "÷" \? "1にんぶん" : "こたえ"/);
