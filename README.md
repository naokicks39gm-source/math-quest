...
- 手書きは「計算メモ」ボタンで開く透明ポップアップ内で利用します（採点には使いません）。
- ポップアップは画面下部に表示され、問題カードを隠さない配置です。
- 計算メモポップアップ内は 2 本指ピンチで拡大縮小できます。`100%` で表示倍率をリセットできます。
- 将来の再利用用に旧手書き判定画面を `/quest-handwrite-legacy` に退避しています（通常導線には表示しません）。

## 出題ストック方式（高速化）

- 出題は「その場生成」ではなく、`type_id` ごとに最大50題を事前ストックしてから使います。
- 実際のクイズ開始時は、ストックから5題を非復元抽出（同一セット重複ゼロ）します。
- 50題に満たないタイプは、作れた数だけストックして不足理由を保持します。
- 5題未満のタイプは `候補不足タイプ一覧` に `typeId / count / reason` を表示します。
- `count >= 5` のタイプは常に5題で開始し、`blocked` にはしません。
